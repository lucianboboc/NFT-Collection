...
const Home = () => {
  ...
  const [tokenIdsMinted, setTokenIdMinted] = useState("0");
  ...
  const publicMint = async () => {
    try {
      ...
      const tx = whitelistContract.publicMint({
        // value signifies the cost of one crypto dev which is "0.01" eth.
        // We are parsing `0.01` string to ether using the utils library from ethers.js
        value: utils.parseEther("0.01"),
      });
  ...
  */
  ...
  const checkIfPresaleEnded = async () => {
    try {
      ...
      const hasEnded = _presaleEnded.lt(Math.floor(Date.now() / 1000.0));
      if (hasEnded) {
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 4) {
      window.alert("Rinkeby is required");
      throw new Error("Rinkeby is required");
    }
